...
async function main() {
  ...
  if (args.length === 0) {
    console.error('❌ Error: Template JSON file is required');
    console.log(CLI_HELP);
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`🎬 Processing template: "${template.meta.title}"`);
    console.log(`📊 ${template.dataBindings.insights.length} insights`);
